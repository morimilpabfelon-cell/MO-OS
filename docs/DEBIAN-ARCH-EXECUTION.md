...
- requires the fixed and already-running `mo-dev` domain;
- never starts Arch in response to `arch.status`;
- resolves the machine root and worker paths canonically and rejects intermediate symlinks;
...
- applies bounded state and execution timeouts;
- validates the exact output schema and Arch identity;
- emits normalized JSON.

A stopped domain or a modified, missing, noncanonical, symlinked or non-executable worker is rejected before the worker is invoked. Starting Arch remains an explicit Debian administrative action through `mo dev` or systemd policy, not a side effect of a signed status request.
...
The automated tests verify signatures, Ed25519 key enforcement, replay under a different bundle name, worker integrity, canonical worker paths, stopped-domain rejection, fixed operation dispatch, malformed evidence, wrong-domain evidence and failure semantics.
